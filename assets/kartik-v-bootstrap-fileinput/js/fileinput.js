	...
	FileInput.prototype = {
		...
		renderFileFooter: function(caption, width) {
			var self = this,
				...
				footer, out, template = self.getLayoutTemplate('footer');
			if (self.isUploadable) {
		...
		uploadExtra: function(previewId, index) {
			var self = this,
				data = self.getExtraData(previewId, index);
			if (data.length === 0) {
				return;
			}
		...
		uploadBatch: function() {
			...
			fnError = function(jqXHR, textStatus, errorThrown) {
				var outData = self.getOutData(jqXHR),
					errMsg = self.parseError(jqXHR, errorThrown);
		...
		previewFile: function(i, file, theFile, previewId, data) {
			...
			var self = this,
				...
				content, strText, types = self.allowedPreviewTypes,
